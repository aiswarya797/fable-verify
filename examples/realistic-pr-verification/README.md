...
Run it from the Agent Audits repository root:
...
- `agent-audits show` output before each review;
- `agent-audits review` records with `supports` notes;
- `agent-audits check --json` supervisor output;
- a generated final Markdown report.

The screenshot artifact is deliberately simple and local. Agent Audits records
...
This is the outreach-safe claim: Agent Audits records proof of completion and
