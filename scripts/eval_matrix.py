...
def good_command_evidence_passes(cli: str, cwd: Path) -> tuple[bool, str]:
    ...
    check = run_cli(cli, cwd, "check")
    ok = add.returncode == 0 and check.returncode == 0 and "PASS" in check.stdout
    return ok, "gate passed with command-backed test evidence" if ok else check.stdout or add.stderr
...
def tampered_artifact_fails(cli: str, cwd: Path) -> tuple[bool, str]:
    ...
    ok = add.returncode == 0 and first_check.returncode == 0 and second_check.returncode != 0 and "tampered" in second_check.stdout
    return ok, "gate failed after artifact tampering" if ok else second_check.stdout or add.stderr
...
def missing_artifact_fails(cli: str, cwd: Path) -> tuple[bool, str]:
    ...
    artifact.unlink()
    check = run_cli(cli, cwd, "check")
    ok = add.returncode == 0 and check.returncode != 0 and "artifact is missing" in check.stdout
    return ok, "gate failed after artifact deletion" if ok else check.stdout or add.stderr
